...
describe("tool definitions", () => {
  ...
  it("rejects invalid input and blocks trading tools while paused", () => {
    ...
    const bad = invokeTool(
      ...
      { db, investorId: ID, agent: "atlas", runId: null },
    );
    ...
    invokeTool(
      ...
      { db, investorId: ID, agent: "sentinel", runId: null },
    );
...
describe("offline desk", () => {
  it("runs a full desk cycle: alerts, reviews and a rebalance proposal", async () => {
    ...
    const alerts = listAlerts(db, ID, { openOnly: true });
    expect(alerts.some((a) => a.code === "valuation_self_marked")).toBe(true);
    expect(alerts.some((a) => a.code === "bitcoin_limit")).toBe(true);
    expect(
      listProposals(db, ID, { status: "pending" }).some((p) => p.title.startsWith("Rebalance")),
    ).toBe(true);
  });

  it("Scout's routine rejects the shipyard bond", () => {
    const digest = runOfflineRoutine(
      "scout",
      { db, investorId: ID, agent: "scout", runId: null },
      emit,
    );
...
describe("offline copilot", () => {
  const ask = (q: string) =>
    offlineCopilot(q, { db, investorId: ID, agent: "copilot", runId: null }, emit);
  ...
  it("answers liquidity, refuses withdrawals and proposes trades", () => {
    ...
    expect(listProposals(db, ID, { status: "pending" })).toHaveLength(1);
